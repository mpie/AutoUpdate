...
import urllib,urllib2,re,os,shutil,threading,datetime,time,base64,zipfile,xbmc,xbmcplugin,xbmcgui,xbmcaddon,xbmcvfs
...
class main:
    def __init__(self):
        ...
        try:        catid = urllib.unquote_plus(params["catid"])
        except:     catid = None
        ...
        # series
        elif action == 'root_shows':                  shows().popular()
        # live tv
        elif action == 'root_livetv':                 thai().tv()
        elif action == 'thai_playtv':                 thai().playtv(name, url, image)
        # thai shows
        elif action == 'root_thai':                   thai().thai_shows()
...
class index:
    ...
    def settings_reset(self):
        try:
            if SettingsVersion == '2.0.0': return
    ...
    def addonArt(self, image, root=''):
        if image.startswith('http://'):
            ...
            elif root == 'root_doofree': image = 'DefaultVideoPlaylists.png'
    ...
    def showLiveTvList(self, channelList):
        for item in channelList['list']:
            name = item['title'].encode('utf-8')
            u = item['location']
            action = 'thai_playtv'
            image = item['thumbnail']
            resolver = item['resolver']
            channelUrl = self.build_url({'action': action, 'name': name, 'url': u, 'image': image, 'resolver': resolver})
            item = xbmcgui.ListItem(name, iconImage="DefaultVideo.png", thumbnailImage=image)
            item.setProperty("Fanart_Image", self.addonArt('fanart.jpg'))
            item.setInfo(type="Video", infoLabels={ "Title": name, "OriginalTitle": name })
            ok = xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=channelUrl, listitem=item, isFolder=False)
    ...
    def showThaiCategoriesList(self, catList):
        for cat in catList:
            name = cat['title']
            u = link().thaishows_baseurl % (cat['id'])
            action = 'thai_shows_list'
            showUrl = self.build_url({'action': action, 'name': name, 'url': u, 'catid': cat['id']})
            item = xbmcgui.ListItem(name, iconImage="DefaultVideo.png", thumbnailImage="")
            item.setProperty("Fanart_Image", self.addonArt('fanart.jpg'))
            item.setInfo(type="Video", infoLabels={"Title": name, "OriginalTitle": name})
            ok = xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=showUrl, listitem=item, isFolder=True)
    ...
    def showThaiShowsList(self, showsList):
        for show in showsList:
            name = show['name'].encode('utf-8')
            url = show['url']
            image = show['image']
            catid = show['catid']
            action = 'thai_shows_episodes'
            showUrl = self.build_url({'action': action, 'name': name, 'url': url, 'catid': catid, 'image': image})
            item = xbmcgui.ListItem(name, iconImage="DefaultVideo.png", thumbnailImage=image)
            item.setProperty("Fanart_Image", self.addonArt('fanart.jpg'))
            item.setInfo(type="Video", infoLabels={"Title": name, "OriginalTitle": name})
            ok = xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=showUrl, listitem=item, isFolder=True)
    ...
    def showThaiEpisodesList(self, episodesList):
        for episode in episodesList:
            name = episode['name'].encode('utf-8')
            url = episode['url']
            image = episode['image']
            channel = episode['channel']
            action = 'thai_shows_play_episode'
            episodeUrl = self.build_url({'action': action, 'name': name, 'url': url, 'channel': channel, 'image': image})
            item = xbmcgui.ListItem(name, iconImage="DefaultVideo.png", thumbnailImage=image)
            item.setProperty("Fanart_Image", self.addonArt('fanart.jpg'))
            item.setInfo(type="Video", infoLabels={"Title": name, "OriginalTitle": name})
            ok = xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=episodeUrl, listitem=item, isFolder=False)
    ...
    def movieList(self, movieList):
        ...
        try:
            ...
            u = '%s?action=movies_latest&url=%s' % (sys.argv[0], urllib.quote_plus(url))
...
class contextMenu:
    ...
    def library_tv_processor(self, name, year, imdb, tvdb, limit=False):
        ...
        for i in match:
            try:
                ...
                if int(re.sub('[^0-9]', '', str(date))) == int(dt.strftime("%Y%m%d")):
                    try: src = resolver().sources_tv(name, title, year, imdb, tvdb, date, season, episode, show, show_alt)
                    except: src = []
    ...
    def download(self, name, url, provider):
        try:
            ...
            commondownloader.download(url, dest, 'DooFree', referer=referer, agent=agent, cookie=cookie)
        except:
            return
...
class root:
    ...
    def get(self):
        ...
        rootList.append({'name': 30501, 'image': 'root_movies.jpg', 'action': 'movies_latest', 'url': latestMovies})
    ...
    # Movies dashboard - skipping this to show latest movies instead!
    def movies(self):
        rootList = []
...
class link:
    def __init__(self):
        ...
        self.imdb_tv_popular = 'http://www.imdb.com/search/title?title_type=tv_series,mini_series&sort=moviemeter,asc&count=100&start=1'
        ...
        self.thaishows_items.append({'id': 18, 'title': 'ละครไทย (ออนแอร์) / Thai Dramas (on air)'})
        self.thaishows_items.append({'id': 27, 'title': 'ละครไทย (อวสาน) / Thai Dramas (ended)'})
        self.thaishows_items.append({'id': 17, 'title': 'ซีรี่ย์เกาหลี / Korean Series'})
        self.thaishows_items.append({'id': 37, 'title': 'หนังจีนชุด / Chinese Series'})
        self.thaishows_items.append({'id': 15, 'title': 'รายการอาหาร / Cooking Shows'})
        self.thaishows_items.append({'id': 8, 'title': 'วาไรตี้โชว์ / Variety Shows'})
        self.thaishows_items.append({'id': 84, 'title': 'เรียลลิตี้โชว์ / Reality & Singing Contest'})
        self.thaishows_items.append({'id': 4, 'title': 'ข่าว / Thai News'})
        self.thaishows_items.append({'id': 14, 'title': 'ข่าวบันเทิง / Thai Entertainment News'})
        self.thaishows_items.append({'id': 92, 'title': 'ภาพยนตร์ไทย / Thai Movies'})
        self.thaishows_items.append({'id': 98, 'title': 'ภาพยนตร์ฝรั่งใหม่ / US Movies (Thai dubbed)'})
        self.thaishows_items.append({'id': 38, 'title': 'ซีรี่ย์ฝรั่ง / US Series (Thai dubbed)'})
        self.thaishows_items.append({'id': 93, 'title': 'ภาพยนตร์แอนนิเมชั่น / Animation'})

        self.tmdb_base = 'http://api.themoviedb.org'
        # 57983e31fb435df4df77afb854740ea9
...
class movies:
    ...
    def latest(self, url):
        self.list = index().cache(self.icefilms_list, 24, url)
    ...
    def iwatchonline_list(self, url):
        ...
        try:
            next = common.parseDOM(result, "li", attrs = { "class": "next pagea" })[0]
        ...
        for movie in movies:
            try:
                title = re.compile('title hide">(.+?) <div class="pull-right">').findall(movie)[0]
                title = common.replaceHTMLCodes(title).encode('utf-8').strip()
                year = re.compile('(\d{4})').findall(title)[0]
                year = year.encode('utf-8')
                ...
                imdb = re.sub('[^0-9]', '', url.rsplit('/', 1)[-1])
                imdb = imdb.encode('utf-8')
                ...
                try: rating = common.parseDOM(movie, "div", attrs = { "data-rating": ".+?" })[0]
                except: rating = '0'
                if rating == '': rating = '0'
                ...
                try: plot = common.parseDOM(movie, "div", attrs = { "class": "shortdescription description hide" })[0]
                except: plot = '0'
                if plot == '': plot = '0'
                plot = common.replaceHTMLCodes(plot)
                plot = plot.encode('utf-8')
    ...
    def icefilms_list(self, url):
        ...
        dialogWait.update(0, '[B]Fetching the newest HD movies[/B]', remaining_display)
        for i in range(0, totalLinks):
            loadedLinks = loadedLinks + 1
            percent = (loadedLinks * 100)/totalLinks
            remaining_display = 'Movies loaded :: [B]'+str(loadedLinks)+' / '+str(totalLinks)+'[/B].'
            dialogWait.update(percent,'[B]Fetching the newest HD movies[/B]', remaining_display)
    ...
    def imdb_list(self, url):
        ...
        for movie in movies:
            try:
                ...
                print {'name': name, 'title': title, 'year': year, 'imdb': imdb, 'tvdb': '0', 'season': '0', 'episode': '0', 'show': '0', 'show_alt': '0', 'date': '0', 'genre': genre, 'url': url, 'poster': poster, 'fanart': '0', 'studio': '0', 'duration': duration, 'rating': rating, 'votes': votes, 'mpaa': mpaa, 'director': director, 'plot': plot, 'plotoutline': tagline, 'tagline': tagline, 'next': next}
                self.list.append({'name': name, 'title': title, 'year': year, 'imdb': imdb, 'tvdb': '0', 'season': '0', 'episode': '0', 'show': '0', 'show_alt': '0', 'date': '0', 'genre': genre, 'url': url, 'poster': poster, 'fanart': '0', 'studio': '0', 'duration': duration, 'rating': rating, 'votes': votes, 'mpaa': mpaa, 'director': director, 'plot': plot, 'plotoutline': tagline, 'tagline': tagline, 'next': next})
            except:
                pass
...
class thai:
    ...
    def thai_shows(self):
        index().showThaiCategoriesList(link().thaishows_items)
        return self.list

    def thai_shows_list(self, url, catid):
        self.list = index().cache(self.shows_list, 1, url, catid)
        index().showThaiShowsList(self.list)
        return self.list

    def thai_shows_episodes(self, url, catid):
        self.list = index().cache(self.episodes_list, 1, url, catid)
        index().showThaiEpisodesList(self.list)
        return self.list

    def thai_shows_play_episode(self, name, url, channel):
        dialog = xbmcgui.DialogProgress()
        dialog.create('One moment', 'Resolving link')
        dialog.update(0)
    ...
    def shows_list(self, url, catid):
        limatch = []
        webPage = link().thaishows_pageurl % (catid)
        pageContent = getUrl(webPage, timeout='10').result
        ...
        for page in pageMatch:
            if (page.isdigit()):
                ...
                pageLink = getUrl(pageUrl, timeout='10').result
        ...
        for licontent in limatch:
            ...
            u = "http://www.seesantv.com/seesantv_2014/" + show[0][0]
            image = show[0][1]
            self.list.append({'name': title, 'url': u, 'image': image, 'catid': catid})
        return self.list

    def episodes_list(self, url, catid):
        catid = int(catid)
        ...
        for episode in episodes:
            if (episode[1].find('(HD)') != -1 and (catid == 18 or catid == 27)) or (catid not in [18, 27]):
                ...
                image = image
                self.list.append({'name': name, 'url': u, 'image': image, 'channel': channel})
        return self.list
        ...
        if (trySD and found==False):
            for host in uk:
                ...
                if (found):
                    ...
                    break
        # try asia host
        if (found==False and channel not in customChannels):
            for host in asia:
...
class resolver:
    ...
    def run(self, name, title, year, imdb, tvdb, season, episode, show, show_alt, date, genre, url):
        dialogWait = xbmcgui.DialogProgress()
        ret = dialogWait.create('Have fun!')
        totalLinks = len(self.sources)
        loadedLinks = 0
        remaining_display = 'Fetching the best available movie and subtitle for you!'
        dialogWait.update(0, '[B]Get ready![/B]', remaining_display)
